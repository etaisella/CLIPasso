...
class Painter(torch.nn.Module):
    def __init__(self, args,
        ...
        if self.doPixelArt:
            ...
            resized_ref[:] = Z[:]
            print(resized_ref.min())
            resized_ref = cv.resize(resized_ref, (3, self.canvas_height, self.canvas_width))
            ...
            if self.doColorQuantization:
                ...
                print(torch.unsqueeze(torch.tensor(resized_ref)).size())
                self.pixelArtImg = torch.nn.Parameter(torch.unsqueeze(torch.tensor(resized_ref), -1) * (self.scaleMax - self.scaleMin) - self.scaleMax, requires_grad=True)
    ...
    def quantize_image(self, descaled):
        ...
        quantized_img = torch.clamp(quantized_img, min=0.0, max=1.0)
...
class PainterOptimizer:
    def __init__(self, args, renderer):
        ...
        self.color_center_lr = args.centroidLr
    ...
    def update_lr(self, counter):
        if (((counter + 1) % 300) == 0) and (counter < 5000):
            self.scheduler.step()
            print("Sched Step")
